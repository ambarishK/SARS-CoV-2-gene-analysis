            ...
            for mutations in rows[primer_name + "_mutations"].values:
                for m in literal_eval(mutations):
                    if m["position"] in values:
                        ...
                        values[m["position"]] = 1
            hist_dict_normalized[primer_name + "/" + country] = [values[i] / len(rows) if i in values else 0 for i in
                                                                 range(len(primers[primer_name]))]
            hist_dict[primer_name + "/" + country] = [values[i] if i in values else 0 for i in
                                                      range(len(primers[primer_name]))]
...
def create_differences(hist_dict: {str: [int]}) -> {str: int}:
    ...
    for test in primers:
        for country1, country2 in itertools.combinations(pcr_data['country'].unique(), 2):
            differences[f"{test}/{country1}.{test}/{country2}"] = sum(
                (abs(a - b) for a, b in zip(hist_dict[f"{test}/{country1}"], hist_dict[f"{test}/{country2}"])))
    return differences


differences = create_differences(hist_dict)
top_differences = dict(sorted(differences.items(), key=operator.itemgetter(1), reverse=True))
...
def hist_comparison(data, country1, country2):
    ...
    plt.title(country2)
    plt.tight_layout()
    plt.savefig('../../plots/countries_comparison/' + country1.replace('/','_')+'_vs_' +country2.replace('/','_'))
...
for x in top_differences:
    ...
    if '_F' in x.split('.')[0] and '_F' in x.split('.')[1] and x.split('.')[0]:
        hist_comparison(hist_df, x.split('.')[0], x.split('.')[1])
        already_was.append(x.split('.')[0])
        already_was.append(x.split('.')[1])
